...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Заглушка: здесь будет логика аутентификации (API call)
      await new Promise((res) => setTimeout(res, 800));
      // Для демонстрации просто логинится любой ввод
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white dark:bg-gray-900 p-6 rounded-lg shadow">
        <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Вход</h2>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => { setEmail(''); setPassword(''); }} className="px-4 py-2 border rounded">Сброс</button>
        </div>
      </form>
    </div>
  );
